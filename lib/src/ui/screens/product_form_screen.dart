...
class _ProductFormScreenState extends State<ProductFormScreen> {
  ...
  void initState() {
    ...
    _skuController = TextEditingController(
        text: p?.sku ?? 'KEY-${DateTime.now().millisecondsSinceEpoch}');
    _quantityController =
        TextEditingController(text: p?.quantity.toString() ?? '0');
    _alertLevelController =
        TextEditingController(text: p?.alertLevel.toString() ?? '2');
    _costPriceIqdController =
        TextEditingController(text: p?.costPriceIqd.toString() ?? '0.0');
    _sellPriceIqdController =
        TextEditingController(text: p?.sellPriceIqd.toString() ?? '0.0');
    _costPriceUsdController =
        TextEditingController(text: p?.costPriceUsd.toString() ?? '0.0');
    _sellPriceUsdController =
        TextEditingController(text: p?.sellPriceUsd.toString() ?? '0.0');
    _oemPartNumberController =
        TextEditingController(text: p?.oemPartNumber ?? '');
    _compatiblePartNumberController =
        TextEditingController(text: p?.compatiblePartNumber ?? '');
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_isSaving)
            ...
            IconButton(
              icon: const Icon(Symbols.save, weight: 700),
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration:
              const InputDecoration(labelText: 'SKU (رقم المنتج)'),
            ...
            TextFormField(
              ...
              decoration:
              const InputDecoration(labelText: 'رقم القطعة الأصلي (OEM)'),
            ),
            const SizedBox(height: 16),
            TextFormField(
              controller: _compatiblePartNumberController,
              decoration: const InputDecoration(
                  labelText: 'أرقام القطع المتوافقة (بينها ,)'),
            ),
            const SizedBox(height: 24),
            ...
            Consumer<SupplierNotifier>(
              builder: (context, notifier, child) {
                ...
                notifier.suppliers.map((s) => s.id).toSet();
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration:
                    const InputDecoration(labelText: 'التكلفة (دينار)'),
                    keyboardType:
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    decoration:
                    const InputDecoration(labelText: 'التكلفة (دولار)'),
                    keyboardType:
                    const TextInputType.numberWithOptions(decimal: true),
                  ),
                ),
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration:
                    const InputDecoration(labelText: 'البيع (دينار)'),
                    keyboardType:
                    const TextInputType.numberWithOptions(decimal: true),
                  ),
                ),
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    decoration:
                    const InputDecoration(labelText: 'البيع (دولار)'),
                    keyboardType:
                    const TextInputType.numberWithOptions(decimal: true),
                  ),
                ),
          ...
          Card(
            ...
            child: Container(
              ...
              child: _imageFile != null
                ...
                imageUrl: githubService
                    .getImageUrl(widget.product!.imagePath!),
                httpHeaders:
                githubService.authHeaders,
                fit: BoxFit.cover,
                placeholder: (c, u) =>
                const Center(child: CircularProgressIndicator()),
                errorWidget: (c, u, e) =>
                const Icon(Symbols.broken_image, size: 64, weight: 700),
              )
                  : const Center(child: Icon(Symbols.key, size: 64, weight: 700))),
            ),
          ),
          const SizedBox(height: 16),
          Row(
            ...
            children: [
              TextButton.icon(
                icon: const Icon(Symbols.photo_camera, weight: 700),
                label: const Text('الكاميرا'),
                onPressed: () => _pickImage(ImageSource.camera),
              ),
              const SizedBox(width: 16),
              TextButton.icon(
                icon: const Icon(Symbols.photo_library, weight: 700),
